...
import fr.ekito.myweatherapp.view.Event.ErrorEvent
import fr.ekito.myweatherapp.view.Event.LoadingEvent
import fr.ekito.myweatherapp.view.Event.SuccessEvent
...
class SplashViewModelMockTest {
    ...
    fun testGetLasttWeatherFailed() {
        ...
        Assert.assertEquals(2, values.size)
        Assert.assertEquals(LoadingEvent, values[0])
        Assert.assertEquals(ErrorEvent(error), values[1])
    }
}